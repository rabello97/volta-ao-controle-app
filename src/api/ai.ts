...
export function getMonthlyInsight(scope?: string): Promise<MonthlyInsight | null> {
  return apiRequest<MonthlyInsight | null>("/ai/monthly", { query: { scope } }).then((r) => r ?? null);
}

export function generateMonthlyInsight(scope?: string): Promise<MonthlyInsight> {
  return apiRequest<MonthlyInsight>("/ai/monthly", { method: "POST", body: { scope } });
}
