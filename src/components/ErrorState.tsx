...
export function ErrorState({
  ...
  return (
    <section className="flex flex-col items-center gap-2 rounded-[18px] border border-divider bg-surface px-[22px] py-12 text-center">
      ...
      <span className="max-w-[380px] text-[12.5px] leading-[1.5] text-text-3">{description}</span>
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="mt-3 flex items-center gap-[7px] rounded-[10px] bg-brand px-4 py-[9px] text-[12.5px] font-semibold text-brand-ink transition-all hover:bg-brand-hover active:scale-95"
        >
